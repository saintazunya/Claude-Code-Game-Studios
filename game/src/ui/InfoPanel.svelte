  ...
  <div class="bg-[#111827] w-full max-w-[430px] min-h-dvh p-5" onclick={(e) => e.stopPropagation()}>
    ...
    <div class="bg-[#1a2234] rounded-xl p-4 border border-[#2a3050] mb-3">
      ...
      <div class="grid grid-cols-2 gap-y-2 text-xs">
        ...
        {#if gs.immigration.i140Status === 'approved' && gs.immigration.priorityDate !== null}
          <span class="text-gray-400">表B(Filing)</span><span class="text-right {gs.immigration.chartBCurrent >= gs.immigration.priorityDate ? 'text-green-400' : 'text-amber-400'}">{gs.immigration.chartBCurrent >= gs.immigration.priorityDate ? '已到 ✅' : `${Math.round(gs.immigration.chartBCurrent)}/${gs.immigration.priorityDate}`}</span>
          <span class="text-gray-400">表A(Final)</span><span class="text-right {gs.immigration.priorityDateCurrent >= gs.immigration.priorityDate ? 'text-green-400' : 'text-amber-400'}">{gs.immigration.priorityDateCurrent >= gs.immigration.priorityDate ? '已到 ✅' : `${Math.round(gs.immigration.priorityDateCurrent)}/${gs.immigration.priorityDate}`}</span>
        {/if}
