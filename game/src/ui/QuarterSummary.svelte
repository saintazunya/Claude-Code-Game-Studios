<script lang="ts">
  ...
  const EVENT_MESSAGES: Record<string, { icon: string; text: string; color: string; detail?: string }> = {
    ...
    sudden_death: { icon: '💀', text: '过劳猝死', color: 'text-red-600', detail: '长期健康和精神处于极度危险水平，身体终于撑不住了。' },
    health_warning_critical: { icon: '🚨', text: '身体发出严重警告！', color: 'text-red-400', detail: '健康和精神都在危险线以下，如果继续这样下去会有生命危险！赶紧休息！' },
    opt_unemployment_exceeded: { icon: '✈️', text: 'OPT失业超限，被迫离境', color: 'text-red-400', detail: '在OPT期间连续失业超过规定时间，身份失效。' },
    ...
    // Immigration-specific events
    h1b_multiple_registrations: { icon: '🎰', text: 'H1B多家注册', color: 'text-purple-400', detail: '关于多家公司同时注册H1B的决定。' },
    perm_prevailing_wage: { icon: '💰', text: 'Prevailing Wage问题', color: 'text-amber-400', detail: '劳工部定的工资标准过高，可能影响PERM申请。' },
    perm_recruitment_issue: { icon: '📋', text: 'PERM招聘流程问题', color: 'text-amber-400', detail: 'PERM强制招聘环节出了纰漏。' },
    i140_rfe: { icon: '📋', text: 'I-140补件通知', color: 'text-amber-400', detail: 'USCIS要求补充材料。' },
    eb2_eb3_downgrade: { icon: '🔄', text: 'EB2/EB3选择', color: 'text-purple-400', detail: '考虑是否降级到EB3加快排期。' },
    visa_bulletin_big_jump: { icon: '🎉', text: '排期大幅前进！', color: 'text-green-400', detail: '签证公告牌排期前进了一年多！' },
    visa_bulletin_retrogression: { icon: '😤', text: '排期大幅倒退', color: 'text-red-400', detail: '排期突然倒退两年。' },
    layoff_h1b_transfer_panic: { icon: '😱', text: 'H1B转换危机', color: 'text-red-400', detail: '裁员后60天内需要transfer H1B。' },
    i485_medical_rfe: { icon: '🏥', text: 'I-485体检补件', color: 'text-amber-400', detail: '需要重新提交体检报告。' },
    uscis_processing_delay: { icon: '🐢', text: 'USCIS审批延迟', color: 'text-amber-400', detail: '大规模积压，审批时间翻倍。' },
    policy_h1b_wage_increase: { icon: '📜', text: 'H1B工资要求提高', color: 'text-amber-400', detail: '政策变化可能影响续签。' },
    policy_premium_processing_suspended: { icon: '📜', text: '加急审理暂停', color: 'text-amber-400', detail: 'Premium Processing暂停。' },
    travel_visa_stamp_expired: { icon: '✈️', text: '签证stamp过期', color: 'text-amber-400', detail: '出境需要重新贴签。' },
    company_merger_immigration: { icon: '🏢', text: '公司收购影响移民', color: 'text-amber-400', detail: '移民案件需要转到新雇主。' },
    i485_interview_notice: { icon: '📋', text: 'I-485面试通知', color: 'text-blue-400', detail: '绿卡快了！收到面试通知。' },
    forum_success_story: { icon: '📱', text: '论坛Timeline分享', color: 'text-gray-400', detail: '看到别人绿卡批准的故事。' },
    coworker_got_gc: { icon: '🤝', text: '同事拿到绿卡', color: 'text-gray-400', detail: '替他高兴，但心里五味杂陈。' },
